...
class ScanReset(UDSScanner):
    ...
    async def main(self, args: Namespace) -> None:
        ...
        for session in sessions:  # pylint: disable=too-many-nested-blocks
            ...
            self.logger.summary(f"Scanning in session: {g_repr(session)}")
            l_ok[session] = list()
            ...
            for sub_func in range(0x01, 0x80):
                ...
                try:
                    ...
                    self.logger.summary(f"{g_repr(sub_func)}: reset level found!")
        ...
        self.logger.summary(f"ok: {l_ok}")
        self.logger.summary(f"timeout: {l_timeout}")
        self.logger.summary(f"with error: {l_error}")
