...
class ScanIdentifiers(UDSScanner):
    ...
    async def main(self, args: Namespace) -> None:
        ...
        for session in sessions:
            ...
            self.logger.summary(f"Starting scan in session: {g_repr(session)}")
            positive_DIDs = 0
            abnormal_DIDs = 0
            timeout_DIDs = 0
            sub_functions = [0x00]
            ...
            for (DID, sub_function) in product(
                ...
                try:
                    ...
                    self.logger.summary(f"{g_repr(DID)}: Retries exceeded")
                    timeout_DIDs += 1
                    continue
                ...
                if isinstance(resp, NegativeResponse):
                    ...
                    # RequestOutOfRange is a common reply for invalid DataIdentifiers
                        ...
                        self.logger.summary(f"{g_repr(DID)}: {resp}")
                        abnormal_DIDs += 1
                else:
                    self.logger.summary(f"{g_repr(DID)}: {resp}")
                    positive_DIDs += 1

            self.logger.summary(f"Scan in session {g_repr(session)} is complete!")
            self.logger.summary(f"Positive replies: {positive_DIDs}")
            self.logger.summary(f"Abnormal replies: {abnormal_DIDs}")
            self.logger.summary(f"Timeouts: {timeout_DIDs}")
