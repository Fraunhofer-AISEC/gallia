...
class FRDumpConfig(AsyncScriptConfig):
    src_slot: AutoInt | None = Field(description="the source flexray slot")
...
class FRDump(AsyncScript):
    ...
    async def main(self) -> None:
        ...
        if self.config.src_slot or self.config.target_slot:
            tp.add_block_all_filter()

            if self.config.src_slot is not None:
                tp.set_acceptance_filter(self.config.src_slot)

            if self.config.target_slot is not None:
                tp.set_acceptance_filter(self.config.target_slot)
