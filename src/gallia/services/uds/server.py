...
class RandomUDSServer(UDSServer):
    ...
    def randomize(self) -> None:
        ...
        while len(level_sessions) > 0:
            ...
            available_sessions = [i for i, level in enumerate(session_transitions) if len(level) > 0]
...
class UDSServerTransport:
    ...
    async def handle_request(self, request_pdu: bytes) -> tuple[bytes | None, float]:
        ...
        if response is not None:
            logger.debug(f"  <--- {response} after {(end - start) * 1000:.2f} ms")
            return response.pdu, end - start

        logger.debug(f"  x--- NO RESPONSE after {(end - start) * 1000:.2f} ms")
