...
class ISOTPTransport(BaseTransport, scheme="isotp", spec=isotp_spec):
    ...
    async def write(
        ...
        self.logger.write(data.hex(), tags=tags)
    ...
    async def read(
        ...
        try:
            ...
            raise e
        self.logger.read(data.hex(), tags=tags)
        return data
...
class RawCANTransport(BaseTransport, scheme="can-raw", spec=spec_can_raw):
    ...
    async def sendto(
        ...
        if self.args["is_extended"]:
            self.logger.write(f"{dst:08x}#{data.hex()}", tags=tags)
        else:
            self.logger.write(f"{dst:03x}#{data.hex()}", tags=tags)
